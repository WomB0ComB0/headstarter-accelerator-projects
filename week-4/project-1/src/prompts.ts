/**
 * Module for handling prompts and token management for code review conversations.
 * @module prompts
 */
...
/**
 * Maps Groq model identifiers to their maximum token context window sizes.
 */
...
/**
 * System prompt for reviewing pull request diffs.
 * Instructs the model to focus on new code additions and provide constructive feedback.
 */
...
/**
 * System prompt for generating XML-formatted code reviews.
 * Provides detailed instructions on review format and focus areas.
 */
...
/**
 * Template for formatting individual code suggestions.
 */
...
/**
 * Assigns line numbers to a diff string, preserving chunk headers and handling removed lines.
 * @param {string} diff - The git diff string to process
 * @returns {string} Diff with line numbers added
 */
...
/**
 * Builds a suggestion prompt for a single file.
 * @param {PRFile} file - Pull request file to build suggestion for
...
/**
 * Builds a patch prompt using either raw or smart context strategy.
 * @param {PRFile} file - Pull request file to build patch for
 * @returns {string} Formatted patch prompt
 */
...
/**
 * Creates a review prompt conversation array.
 * @param {string} diff - The diff content to review
 * @returns {ChatCompletionMessageParam[]} Array of conversation messages
 */
...
/**
 * Creates an XML-formatted review prompt conversation array.
 * @param {string} diff - The diff content to review
 * @returns {ChatCompletionMessageParam[]} Array of conversation messages
 */
...
/**
 * Constructs a complete conversation prompt from multiple files.
 * @param {PRFile[]} files - Array of pull request files
 * @param {Function} patchBuilder - Function to build patch for each file
 * @param {Function} convoBuilder - Function to build conversation from diff
 * @returns {ChatCompletionMessageParam[]} Complete conversation prompt
 */
...
/**
 * Calculates the token length of a string.
 * @param {string} blob - Text to calculate tokens for
...
/**
 * Checks if a conversation is within the token limit for a given model.
 * @param {any[]} convo - Conversation to check
 * @param {GroqChatModel} model - Model to check against, defaults to GROQ_MODEL
 * @returns {boolean} Whether conversation is within token limit
 */

...
function findAttrInCustomAttributes(name2, attributes) {
  return eff.F.pipe(
    eff.O.fromNullable(attributes.findLast((a) => a.as === name2)),
    eff.O.map((a) => [a.name, a.defaultValue]),
    eff.O.getOrElse(() => [name2])
  );
}
...
var no_dangerously_set_innerhtml_default = createRule({
  ...
  create(context) {
    return {
      JSXElement(node) {
        eff.F.pipe(
          eff.O.some("dangerouslySetInnerHTML"),
          eff.O.flatMap(JSX8__namespace.findPropInAttributes(node.openingElement.attributes, context.sourceCode.getScope(node))),
          eff.O.map((prop) => ({ messageId: "noDangerouslySetInnerhtml", node: prop })),
...
function firstChildIsText(node) {
  ...
  return node.children.length > 0 && !eff.isNullable(firstChild) && !JSX8__namespace.isLineBreak(firstChild);
}
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const hasChildrenProp = () => JSX8__namespace.hasProp(node.openingElement.attributes, "children", initialScope);
        const hasDanger = () => JSX8__namespace.hasProp(node.openingElement.attributes, "dangerouslySetInnerHTML", initialScope);
...
var no_missing_button_type_default = createRule({
  ...
  create(context) {
    return {
      JSXElement(node) {
        const elementName = core.getElementRepresentName(node.openingElement, context);
        if (elementName !== "button") return;
        const { attributes } = node.openingElement;
        const initialScope = context.sourceCode.getScope(node);
        const mbProp = JSX8__namespace.findPropInAttributes(attributes, initialScope)("type");
        if (eff.O.isSome(mbProp)) return;
        context.report({
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const { attributes } = node.openingElement;
        const initialScope = context.sourceCode.getScope(node);
        const mbProp = JSX8__namespace.findPropInAttributes(attributes, initialScope)("sandbox");
        if (eff.O.isNone(mbProp)) {
          context.report({
        ...
        const hasValidSandbox = eff.F.pipe(
          JSX8__namespace.getPropValue(prop, context.sourceCode.getScope(prop)),
          eff.O.filter(eff.isString),
          eff.O.map((value) => value.split(" ")),
          eff.O.exists((values) => values.every((value) => validTypes.some((validType) => validType === value)))
        );
  ...
  create(context) {
    return {
      JSXOpeningElement(node) {
        const name2 = JSX8__namespace.getElementName(node);
        if (!eff.isString(name2) || !name2.includes(":")) return;
        context.report({
  ...
  create(context) {
    return {
      JSXAttribute(node) {
        ...
        const isJavaScript = eff.F.pipe(
          JSX8__namespace.getPropValue(node, context.sourceCode.getScope(node)),
          eff.O.filter(eff.isString),
          eff.O.exists((v) => shared.RE_JAVASCRIPT_PROTOCOL.test(v))
        );
        if (isJavaScript) {
          context.report({
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const initialScope = context.sourceCode.getScope(node);
        const mbProp = JSX8__namespace.findPropInAttributes(attributes, initialScope)("sandbox");
        if (eff.O.isNone(mbProp)) return;
        const prop = mbProp.value;
        const isSafeSandboxValue = !eff.F.pipe(
          JSX8__namespace.getPropValue(prop, context.sourceCode.getScope(prop)),
          eff.O.flatMapNullable(
            (v) => tsPattern.match(v).with(tsPattern.P.string, eff.F.identity).with({ sandbox: tsPattern.P.string }, ({ sandbox }) => sandbox).otherwise(eff.F.constNull)
          ),
          eff.O.filter(eff.isString),
          eff.O.map((value) => value.split(" ")),
          eff.O.exists(
...
var no_unsafe_target_blank_default = createRule({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      const name2 = JSX8__namespace.getElementName(node.openingElement);
      const elementName = core.getElementRepresentName(node.openingElement, context);
      if (elementName !== "a" && !additionalComponents.some((c) => c.re.test(name2))) return eff.O.none();
      const { attributes } = node.openingElement;
      const initialScope = context.sourceCode.getScope(node);
      ...
      const targetProp = JSX8__namespace.findPropInAttributes(attributes, initialScope)(targetPropName);
      const targetPropValue = eff.O.isNone(targetProp) ? eff.O.fromNullable(targetPropDefaultValue) : eff.F.pipe(
        targetProp,
        eff.O.flatMap((attr) => JSX8__namespace.getPropValue(attr, context.sourceCode.getScope(attr))),
        eff.O.flatMapNullable(
          (v) => tsPattern.match(v).with(tsPattern.P.string, eff.F.identity).with({ [targetPropName]: tsPattern.P.string }, (v2) => v2[targetPropName]).otherwise(eff.F.constNull)
        ),
        eff.O.filter(eff.isString)
      );
      if (!eff.O.exists(targetPropValue, (t) => t === "_blank")) return eff.O.none();
      const [
        hrefPropName,
        hrefPropDefaultValue
      ] = findAttrInCustomAttributes("href", additionalAttributes);
      const hrefProp = JSX8__namespace.findPropInAttributes(attributes, initialScope)(hrefPropName);
      const hrefPropValue = eff.O.isNone(hrefProp) ? eff.O.fromNullable(hrefPropDefaultValue) : eff.F.pipe(
        hrefProp,
        eff.O.flatMap((attr) => JSX8__namespace.getPropValue(attr, context.sourceCode.getScope(attr))),
        eff.O.flatMapNullable(
          (v) => tsPattern.match(v).with(tsPattern.P.string, eff.F.identity).with({ [hrefPropName]: tsPattern.P.string }, (v2) => v2[hrefPropName]).otherwise(eff.F.constNull)
        ),
        eff.O.filter(eff.isString)
      );
      if (!eff.O.exists(hrefPropValue, isExternalLinkLike)) return eff.O.none();
      const [
        relPropName,
        relPropDefaultValue
      ] = findAttrInCustomAttributes("rel", additionalAttributes);
      const relProp = JSX8__namespace.findPropInAttributes(attributes, initialScope)(relPropName);
      const relPropValue = eff.O.isNone(relProp) ? eff.O.fromNullable(relPropDefaultValue) : eff.F.pipe(
        relProp,
        eff.O.flatMap((attr) => JSX8__namespace.getPropValue(attr, context.sourceCode.getScope(attr))),
        eff.O.flatMapNullable(
          (v) => tsPattern.match(v).with(tsPattern.P.string, eff.F.identity).with({ [relPropName]: tsPattern.P.string }, (v2) => v2[relPropName]).otherwise(eff.F.constNull)
        ),
        eff.O.filter(eff.isString)
      );
      if (eff.O.exists(relPropValue, isSafeRel)) return eff.O.none();
      return eff.O.some({
...
var no_void_elements_with_children_default = createRule({
  ...
  create(context) {
    return {
      JSXElement(node) {
        const elementName = JSX8__namespace.getElementName(node.openingElement);
        ...
        const { attributes } = node.openingElement;
        const initialScope = context.sourceCode.getScope(node);
        const hasAttr = (name2) => eff.O.isSome(JSX8__namespace.findPropInAttributes(attributes, initialScope)(name2));

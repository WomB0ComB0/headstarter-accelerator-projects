import { F, O, isString, isNullable } from '@eslint-react/eff';
import * as JSX8 from '@eslint-react/jsx';
import { createRuleForPlugin, RE_JAVASCRIPT_PROTOCOL, getSettingsFromContext } from '@eslint-react/shared';
    ...
    return {
      JSXElement(node) {
        F.pipe(
          ...
          O.flatMap(JSX8.findPropInAttributes(node.openingElement.attributes, context.sourceCode.getScope(node))),
          O.map((prop) => ({ messageId: "noDangerouslySetInnerhtml", node: prop })),
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const initialScope = context.sourceCode.getScope(node);
        const mbProp = JSX8.findPropInAttributes(attributes, initialScope)("type");
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const { attributes } = node.openingElement;
        const initialScope = context.sourceCode.getScope(node);
        const mbProp = JSX8.findPropInAttributes(attributes, initialScope)("sandbox");
        if (O.isNone(mbProp)) {
          context.report({
        ...
        const hasValidSandbox = F.pipe(
          JSX8.getPropValue(prop, context.sourceCode.getScope(prop)),
          O.filter(isString),

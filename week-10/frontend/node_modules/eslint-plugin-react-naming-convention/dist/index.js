    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        for (const { node: component } of functionComponents.values()) {
          const mbId = AST__namespace.getFunctionIdentifier(component);
          if (eff.O.isNone(mbId)) continue;
          const id = mbId.value;
        ...
        for (const { node: component } of classComponents.values()) {
          eff.F.pipe(
            AST__namespace.getClassIdentifier(component),
            eff.O.filter((id) => !validate(id.name, options)),
            eff.O.map((id) => {
              context.report({
                messageId: "componentName",
                node: id,
      ...
      "Program:exit"(node) {
        ...
        for (const { hookCalls } of components.values()) {
          ...
          for (const hookCall of hookCalls) {
            if (!core.isUseStateCall(hookCall, context) && !alias.some(core.isReactHookCallWithNameLoose(hookCall))) continue;
            if (hookCall.parent.type !== types.AST_NODE_TYPES.VariableDeclarator) continue;

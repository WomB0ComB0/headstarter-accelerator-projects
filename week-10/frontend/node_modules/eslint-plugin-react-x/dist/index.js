...
var avoid_shorthand_boolean_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      return node.value ? eff.O.none() : eff.O.some({
    ...
    return {
      JSXAttribute: eff.F.flow(getReportDescriptor, eff.O.map(context.report))
    };
  },
  defaultOptions: []
});
  ...
  create(context) {
    ...
    return {
      ...
      MemberExpression(node) {
        ...
        if (!eff.O.exists(getName(node.property), (name2) => name2 === "state")) return;
      ...
      VariableDeclarator(node) {
        ...
        const hasState = node.id.properties.some((prop) => {
          if (prop.type === types.AST_NODE_TYPES.Property && AST11__namespace.isKeyLiteralLike(prop, prop.key)) {
            return eff.O.exists(getName(prop.key), (name2) => name2 === "state");
          }
          return false;
        });
        if (!hasState) return;
...
function isUsingReactChildren(node, context) {
  ...
  if (callee.object.type === types.AST_NODE_TYPES.MemberExpression && "name" in callee.object.object) {
    return core.isInitializedFromReact(callee.object.object.name, initialScope, settings.importSource);
  }
  return false;
}
function getMapIndexParamName(node, context) {
  const { callee } = node;
  if (callee.type !== types.AST_NODE_TYPES.MemberExpression) return eff.O.none();
  if (callee.property.type !== types.AST_NODE_TYPES.Identifier) return eff.O.none();
  const { name: name2 } = callee.property;
  if (!iteratorFunctionIndexParamPosition.has(name2)) return eff.O.none();
  const callbackArg = node.arguments[isUsingReactChildren(node, context) ? 1 : 0];
  if (!callbackArg) return eff.O.none();
  ...
  if (eff.isNullable(indexParamPosition)) return eff.O.none();
  if (params.length < indexParamPosition + 1) return eff.O.none();
  const param = params.at(indexParamPosition);
  return param && "name" in param ? eff.O.some(param.name) : eff.O.none();
}
var no_array_index_key_default = createRule({
  ...
  create(context) {
    ...
    const isCreateOrCloneElementCall = eff.or(core.isCreateElementCall(context), core.isCloneElementCall(context));
    function isArrayIndex(node) {
      return node.type === types.AST_NODE_TYPES.Identifier && indexParamNames.some(eff.O.exists((name2) => name2 === node.name));
    }
    function getReportDescriptor(node) {
  ...
  create(context) {
    return {
      JSXElement(node) {
        ...
        const reportDescriptor = eff.O.map(prop, (prop2) => ({ messageId: "noChildrenProp", node: prop2 }));
        eff.O.map(reportDescriptor, context.report);
      }
    };
  },
  defaultOptions: []
...
var no_comment_textnodes_default = createRule({
  ...
  create(context) {
    ...
    const getReportDescriptor = (node) => {
      if (!AST11__namespace.isOneOf([types.AST_NODE_TYPES.JSXElement, types.AST_NODE_TYPES.JSXFragment])(node.parent)) return eff.O.none();
      if (!hasCommentLike(node)) return eff.O.none();
      if (!node.parent.type.includes("JSX")) return eff.O.none();
      return eff.O.some({
        messageId: "noCommentTextnodes",
        node
      });
    };
    const visitorFunction = eff.F.flow(getReportDescriptor, eff.O.map(context.report), eff.F.constVoid);
    return {
...
var no_complex_conditional_rendering_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      ...
      if (!AST11__namespace.is(types.AST_NODE_TYPES.JSXExpressionContainer)(jsxExpContainer)) return eff.O.none();
      if (!AST11__namespace.isOneOf([types.AST_NODE_TYPES.JSXElement, types.AST_NODE_TYPES.JSXFragment])(jsxExpContainer.parent)) {
        return eff.O.none();
      }
      if (!jsxExpContainer.parent.children.includes(jsxExpContainer)) return eff.O.none();
      return eff.O.some({ messageId: "noComplexConditionalRendering", node: jsxExpContainer });
    }
    const visitorFunction = eff.F.flow(getReportDescriptor, eff.O.map(context.report), eff.F.constVoid);
    return {
...
var no_context_provider_default = createRule({
  ...
  create(context) {
    ...
    return {
      JSXElement(node) {
        const elementName = JSX7__namespace.getElementName(node.openingElement);
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        if (eff.O.isSome(AST11__namespace.findParentNode(node, core.isClassComponent))) return;
...
function isAssignmentToThisState(node) {
  const { left } = node;
  return left.type === types.AST_NODE_TYPES.MemberExpression && AST11__namespace.isThisExpression(left.object) && eff.O.exists(getName2(left.property), (name2) => name2 === "state");
}
...
var no_direct_mutation_state_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      if (!isAssignmentToThisState(node)) return eff.O.none();
      return eff.F.pipe(
        eff.O.Do,
        eff.O.bind("parentClass", () => AST11__namespace.findParentNodeGuard(
          node,
        ...
        eff.O.bind("parentConstructor", () => AST11__namespace.findParentNodeGuard(node, isConstructorFunction)),
        eff.O.filter(
          ({ parentClass, parentConstructor }) => core.isClassComponent(parentClass) && context.sourceCode.getScope(node).block !== parentConstructor
        ),
        eff.O.map(() => ({
    ...
    return {
      AssignmentExpression: eff.F.flow(getReportDescriptor, eff.O.map(context.report))
    };
  },
  defaultOptions: []
});
    ...
    return {
      JSXOpeningElement(node) {
        ...
        for (const attr of node.attributes) {
          ...
          if (!eff.isString(name2)) continue;
    ...
    return {
      "JSXAttribute[name.name='key']"(node) {
        ...
        switch (jsxElement.parent.type) {
          ...
          default: {
            const mbEntry = eff.F.pipe(
              eff.O.Do,
              eff.O.bind("call", () => AST11__namespace.findParentNodeGuard(jsxElement, AST11__namespace.isMapCallLoose)),
              eff.O.bind("iter", ({ call: call2 }) => AST11__namespace.findParentNodeStop(jsxElement, call2, AST11__namespace.isFunction)),
              eff.O.bind("arg0", ({ call: call2 }) => eff.O.fromNullable(call2.arguments[0]))
            );
            if (eff.O.isNone(mbEntry)) return;
            const { arg0, call, iter } = mbEntry.value;
            if (AST11__namespace.unwrapTypeExpression(arg0) !== iter) return;
...
function getFix(node, context) {
  return (fixer) => {
    ...
    if (!componentNode || !AST11__namespace.isFunction(componentNode)) return [];
    return [
...
function getComponentPropsFixes(node, typeArguments, fixer, context) {
  ...
  if (!arg0) return [];
  const fixedArg0Text = tsPattern.match(arg0).with({ type: types.AST_NODE_TYPES.Identifier }, (n) => eff.O.some(`...${n.name}`)).with({ type: types.AST_NODE_TYPES.ObjectPattern }, (n) => eff.O.some(n.properties.map(getText).join(", "))).otherwise(eff.O.none);
  const fixedArg1Text = tsPattern.match(arg1).with(tsPattern.P.nullish, () => eff.O.some("ref")).with({ type: types.AST_NODE_TYPES.Identifier, name: "ref" }, () => eff.O.some("ref")).with({ type: types.AST_NODE_TYPES.Identifier, name: tsPattern.P.not("ref") }, (n) => eff.O.some(`ref: ${n.name}`)).otherwise(eff.O.none);
  if (eff.O.isNone(fixedArg0Text) || eff.O.isNone(fixedArg1Text)) return [];
...
var no_implicit_key_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      ...
      const keyPropOnElement = node.attributes.some((n) => AST11__namespace.is(types.AST_NODE_TYPES.JSXAttribute)(n) && n.name.name === "key");
      if (eff.O.isSome(keyPropFound) && !keyPropOnElement) {
        return eff.O.some({ messageId: "noImplicitKey", node: keyPropFound.value });
      }
      return eff.O.none();
    }
    return {
      JSXOpeningElement: eff.F.flow(getReportDescriptor, eff.O.map(context.report))
    };
  },
  defaultOptions: []
});
...
function inspectVariantTypes(types) {
  ...
  switch (true) {
    case (booleans.length === 1 && !!booleans[0]): {
      ...
      const evaluated = eff.F.pipe(
        tsPattern.match(first).when(tsApiUtils.isTrueLiteralType, () => eff.O.some("truthy boolean")).when(tsApiUtils.isFalseLiteralType, () => eff.O.some("falsy boolean")).otherwise(eff.O.none)
      );
      eff.O.map(evaluated, (v) => variantTypes.add(v));
      break;
    }
  ...
  if (strings.length > 0) {
    const evaluated = tsPattern.match(strings).when((types2) => types2.every(tsHelpers.isTruthyStringType), eff.F.constant("truthy string")).when((types2) => types2.every(tsHelpers.isFalsyStringType), eff.F.constant("falsy string")).otherwise(eff.F.constant("string"));
    variantTypes.add(evaluated);
  }
  const bigints = types.filter(tsHelpers.isBigIntType);
  if (bigints.length > 0) {
    const evaluated = tsPattern.match(bigints).when((types2) => types2.every(tsHelpers.isTruthyBigIntType), eff.F.constant("truthy bigint")).when((types2) => types2.every(tsHelpers.isFalsyBigIntType), eff.F.constant("falsy bigint")).otherwise(eff.F.constant("bigint"));
    variantTypes.add(evaluated);
  }
  const numbers = types.filter(tsHelpers.isNumberType);
  if (numbers.length > 0) {
    const evaluated = tsPattern.match(numbers).when((types2) => types2.every(tsHelpers.isTruthyNumberType), eff.F.constant("truthy number")).when((types2) => types2.every(tsHelpers.isFalsyNumberType), eff.F.constant("falsy number")).otherwise(eff.F.constant("number"));
    variantTypes.add(evaluated);
  }
...
function getVariableInitExpression(at) {
  return (variable) => {
    return eff.F.pipe(
      ...
      eff.O.flatMap(
        (d) => "init" in d.node ? eff.O.fromNullable(d.node.init) : eff.O.none()
      ),
      eff.O.filter(isInitExpression)
    );
  };
}
var no_leaked_conditional_rendering_default = createRule({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      return tsPattern.match(node).when(AST11__namespace.isJSX, eff.O.none).with({ type: types.AST_NODE_TYPES.LogicalExpression, operator: "&&" }, ({ left, right }) => {
        ...
        const isLeftNan = tsPattern.isMatching({ type: types.AST_NODE_TYPES.Identifier, name: "NaN" }, left) || eff.O.exists(VAR__namespace.getStaticValue(left, initialScope), (v) => v === "NaN");
        if (isLeftNan) {
          return eff.O.some({
        ...
        return eff.O.orElse(getReportDescriptor(consequent), () => getReportDescriptor(alternate));
      }).with({ type: types.AST_NODE_TYPES.Identifier }, (n) => {
        return eff.F.pipe(
          VAR__namespace.findVariable(n.name, context.sourceCode.getScope(n)),
          eff.O.flatMap(getVariableInitExpression(0)),
          eff.O.flatMap(getReportDescriptor)
        );
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        for (const { node: node2, displayName, flag } of components.values()) {
          ...
          if (eff.O.isSome(AST11__namespace.getFunctionIdentifier(node2))) continue;
          if (!isMemoOrForwardRef) continue;
          if (eff.O.isNone(displayName)) {
            context.report({
  ...
  create(context) {
    ...
    function checkExpression(node) {
      switch (node.type) {
        case types.AST_NODE_TYPES.ConditionalExpression:
          if (!("consequent" in node)) return eff.O.none();
          return eff.O.orElse(checkIteratorElement(node.consequent), () => checkIteratorElement(node.alternate));
        case types.AST_NODE_TYPES.JSXElement:
        case types.AST_NODE_TYPES.JSXFragment:
          return checkIteratorElement(node);
        case types.AST_NODE_TYPES.LogicalExpression:
          if (!("left" in node)) return eff.O.none();
          return eff.O.orElse(checkIteratorElement(node.left), () => checkIteratorElement(node.right));
        default:
          return eff.O.none();
      }
    }
    function checkBlockStatement(node) {
      return AST11__namespace.getNestedReturnStatements(node).reduce((acc, statement) => {
        ...
        return eff.O.isNone(mbDescriptor) ? acc : [...acc, mbDescriptor.value];
      }, []);
    }
    return {
      ...
      CallExpression(node) {
        state.isWithinChildrenToArray ||= core.isChildrenToArrayCall(node, context);
        if (state.isWithinChildrenToArray) return;
        ...
        eff.O.map(checkExpression(fn.body), context.report);
      },
      "CallExpression:exit"(node) {
        if (!core.isChildrenToArrayCall(node, context)) return;
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        for (const { name: componentName, node: component } of functionComponents) {
          ...
          if (eff.O.isNone(componentName)) continue;
          const name2 = componentName.value;
          const isInsideProperty = component.parent.type === types.AST_NODE_TYPES.Property;
          const isInsideJSXPropValue = component.parent.type === types.AST_NODE_TYPES.JSXAttribute || eff.O.isSome(JSX7__namespace.findParentProp(node, (n) => n.value?.type === types.AST_NODE_TYPES.JSXExpressionContainer));
          ...
          const isParnetComponentNotDirectValueOfRenderProperty = eff.O.exists(
            AST11__namespace.findParentNodeGuard(
              component,
              isFunctionComponent
            ),
            eff.not(core.isDirectValueOfRenderPropertyLoose)
          );
        ...
        for (const { name: name2, node: component } of classComponents) {
          if (eff.O.isNone(AST11__namespace.findParentNode(component, eff.or(isClassComponent11, isFunctionComponent)))) {
            continue;
          }
          context.report({
            messageId: "nestedComponent",
            node: component,
            data: {
              name: eff.O.getOrElse(() => "unknown")(name2)
            }
          });
        }
      }
          ...
          for (const member of body) {
            if (isShouldComponentUpdate(member)) {
              context.report({
                ...
                node: member,
                data: {
                  componentName: eff.O.getOrElse(() => "PureComponent")(name2)
                }
              });
            }
          }
...
var no_set_state_in_component_did_mount_default = createRule({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      if (!core.isThisSetState(node)) return eff.O.none();
      return eff.F.pipe(
        eff.O.Do,
        eff.O.bind("clazz", () => AST11__namespace.findParentNodeGuard(node, core.isClassComponent)),
        eff.O.bind("method", ({ clazz }) => AST11__namespace.findParentNodeStop(node, clazz, isComponentDidMount)),
        eff.O.bind("methodScope", ({ method }) => eff.O.some(context.sourceCode.getScope(method))),
        eff.O.bind("upperScope", () => eff.O.fromNullable(context.sourceCode.getScope(node).upper)),
        eff.O.filter(
          ({
            clazz,
            method,
            methodScope,
            upperScope
          }) => method.parent === clazz.body && upperScope === methodScope
        ),
        eff.O.map(() => ({
    ...
    return {
      CallExpression: eff.F.flow(getReportDescriptor, eff.O.map(context.report))
    };
  },
  defaultOptions: []
});
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      ...
      return eff.F.pipe(
        eff.O.Do,
        eff.O.bind("clazz", () => AST11__namespace.findParentNodeGuard(node, core.isClassComponent)),
        eff.O.bind("method", ({ clazz }) => AST11__namespace.findParentNodeStop(node, clazz, isComponentDidUpdate)),
        eff.O.bind("methodScope", ({ method }) => eff.O.some(context.sourceCode.getScope(method))),
        eff.O.bind("upperScope", () => eff.O.fromNullable(context.sourceCode.getScope(node).upper)),
        eff.O.filter(
          ({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      ...
      return eff.F.pipe(
        eff.O.Do,
        eff.O.bind("clazz", () => AST11__namespace.findParentNodeGuard(node, core.isClassComponent)),
        eff.O.bind("method", ({ clazz }) => AST11__namespace.findParentNodeStop(node, clazz, isComponentWillUpdate3)),
        eff.O.bind("methodScope", ({ method }) => eff.O.some(context.sourceCode.getScope(method))),
        eff.O.bind("upperScope", () => eff.O.fromNullable(context.sourceCode.getScope(node).upper)),
        eff.O.filter(
          ({
...
function containsStringLiteral({ value }) {
  return value?.type === types.AST_NODE_TYPES.Literal && eff.isString(value.value);
}
function containsStringExpressionContainer({ value }) {
  if (value?.type !== types.AST_NODE_TYPES.JSXExpressionContainer) return false;
  if (value.expression.type === types.AST_NODE_TYPES.Literal) return eff.isString(value.expression.value);
  ...
  create(context) {
    ...
    return {
      ...
      JSXOpeningElement(node) {
        ...
        eff.F.pipe(
          eff.O.Do,
          eff.O.bind("function", ctx.getCurrentFunction),
          eff.O.bind("attribute", () => eff.O.fromNullable(
            node.attributes.find((attribute) => {
              return attribute.type === types.AST_NODE_TYPES.JSXAttribute && attribute.name.name === "value";
            })
          )),
          eff.O.bind("value", ({ attribute }) => "value" in attribute ? eff.O.some(attribute.value) : eff.O.none()),
          eff.O.bind("valueExpression", ({ value }) => value?.type === types.AST_NODE_TYPES.JSXExpressionContainer ? eff.O.some(value.expression) : eff.O.none()),
          eff.O.bind("construction", ({ valueExpression }) => {
            const initialScope = context.sourceCode.getScope(valueExpression);
            return eff.O.some(VAR__namespace.getValueConstruction(valueExpression, initialScope));
          }),
          eff.O.map((vc) => {
            if (vc.construction.kind === "None") return;
            if (core.isReactHookCall(vc.construction.node)) return;
      ...
      "Program:exit"(node) {
        ...
        for (const { node: component } of components) {
          for (const construction of constructions.get(component) ?? []) {
            ...
            const messageId = kind.startsWith("Function") ? "noUnstableContextValueWithFunction" : "noUnstableContextValueWithIdentifier";
            context.report({
              messageId,
      ...
      "Program:exit"(node) {
        ...
        for (const { node: component } of components.values()) {
          ...
          const properties = tsPattern.match(props).with({ type: types.AST_NODE_TYPES.ObjectPattern }, ({ properties: properties2 }) => properties2).with({ type: types.AST_NODE_TYPES.Identifier }, ({ name: name2 }) => {
            return declarators.get(component)?.filter((d) => d.init.name === name2).flatMap((d) => d.id.properties) ?? [];
          }).otherwise(() => []);
          for (const prop of properties) {
            ...
            if (construction.kind === "None") continue;
            if (core.isReactHookCall(construction.node)) continue;
  ...
  create(context) {
    ...
    return {
      ...
      MemberExpression(node) {
        ...
        eff.O.map(getName3(node.property), (name2) => propertyUsages.get(currentClass)?.add(name2));
      },
      ...
      VariableDeclarator(node) {
        ...
        if (node.init && AST11__namespace.isThisExpression(node.init) && node.id.type === types.AST_NODE_TYPES.ObjectPattern) {
          for (const prop of node.id.properties) {
            if (prop.type === types.AST_NODE_TYPES.Property && AST11__namespace.isKeyLiteralLike(prop, prop.key)) {
              const mbName = getName3(prop.key);
              if (eff.O.isSome(mbName)) {
                const name2 = mbName.value;
                propertyUsages.get(currentClass)?.add(name2);
              }
            }
          }
        }
...
function isAssignmentToThisState2(node) {
  const { left } = node;
  return left.type === types.AST_NODE_TYPES.MemberExpression && AST11__namespace.isThisExpression(left.object) && eff.O.exists(getName4(left.property), (name2) => name2 === "state");
}
...
var no_unused_state_default = createRule({
  ...
  create(context) {
    ...
    function classExit() {
      ...
      if (!currentClass || !core.isClassComponent(currentClass)) return;
      const className = eff.O.map(AST11__namespace.getClassIdentifier(currentClass), (id) => id.name);
      const [def, isUsed] = stateDefs.get(currentClass) ?? [eff.O.none(), false];
      if (eff.O.isNone(def) || isUsed) return;
      context.report({
    ...
    function methodEnter(node) {
      ...
      if (node.static) {
        if (isGetDerivedStateFromProps(node) && node.value.params.length > 1) {
          const [def] = stateDefs.get(currentClass) ?? [eff.O.none()];
          stateDefs.set(currentClass, [def, true]);
        }
        return;
      }
      if (eff.O.exists(getName4(node.key), (name2) => name2 === "state")) {
        stateDefs.set(currentClass, [eff.O.some(node.key), false]);
      }
    }
    function methodExit() {
      methodStack.pop();
    ...
    return {
      AssignmentExpression(node) {
        ...
        const [_, isUsed] = stateDefs.get(currentClass) ?? [eff.O.none(), false];
        stateDefs.set(currentClass, [eff.O.some(node.left), isUsed]);
      },
      ClassDeclaration: classEnter,
      "ClassDeclaration:exit": classExit,
      ClassExpression: classEnter,
      ...
      MemberExpression(node) {
        if (!AST11__namespace.isThisExpression(node.object)) return;
        if (!eff.O.exists(getName4(node.property), (name2) => name2 === "state")) return;
        const currentClass = classStack.at(-1);
        if (!currentClass || !core.isClassComponent(currentClass)) return;
        const currentMethod = methodStack.at(-1);
        ...
        const [def] = stateDefs.get(currentClass) ?? [eff.O.none(), false];
        stateDefs.set(currentClass, [def, true]);
      },
      MethodDefinition: methodEnter,
      "MethodDefinition:exit": methodExit,
      ...
      VariableDeclarator(node) {
        ...
        const hasState = node.id.properties.some((prop) => {
          if (prop.type === types.AST_NODE_TYPES.Property && AST11__namespace.isKeyLiteralLike(prop, prop.key)) {
            return eff.O.exists(getName4(prop.key), (name2) => name2 === "state");
          }
          return false;
        });
        if (!hasState) return;
...
function checkAndReport(node, context, allowExpressions) {
  const initialScope = context.sourceCode.getScope(node);
  ...
  switch (true) {
    ...
    case (allowExpressions && !isChildElement && node.children.length === 1 && JSX7__namespace.isLiteral(node.children.at(0))): {
      return;
    }
    // <Foo><>hello, world</></Foo>
...
var no_useless_fragment_default = createRule({
  ...
  create(context, [option]) {
    ...
    return {
      JSXElement(node) {
        if (JSX7__namespace.getElementName(node.openingElement).split(".").at(-1) !== "Fragment") return;
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        function isFunctionComponent(block) {
          ...
          return eff.O.exists(
            AST11__namespace.getFunctionIdentifier(block),
            (id) => core.isComponentName(id.name) && components.some((component) => component.node === block)
          );
        }
...
var prefer_react_namespace_import_default = createRule({
  ...
  name: RULE_NAME46,
  create(context) {
    const importSource = shared.getSettingsFromContext(context).importSource ?? "react";
    return {
...
var prefer_shorthand_boolean_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      ...
      if (!hasValueTrue) return eff.O.none();
      return eff.O.some({
...
var prefer_shorthand_fragment_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      if (JSX7__namespace.getElementName(node.openingElement).split(".").at(-1) !== "Fragment") return eff.O.none();
      const hasAttributes = node.openingElement.attributes.length > 0;
      if (hasAttributes) return eff.O.none();
      return eff.O.some({
    ...
    return {
      JSXElement: eff.F.flow(getReportDescriptor, eff.O.map(context.report))
    };
  },
  defaultOptions: []
});
  ...
  create(context) {
    ...
    return {
      JSXOpeningElement(node) {
        ...
        for (const name2 of eff.O.toArray(getName5(node.name))) {

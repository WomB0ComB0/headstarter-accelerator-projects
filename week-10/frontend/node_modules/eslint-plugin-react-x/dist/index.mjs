import { F, O, or, isString, not, isNullable } from '@eslint-react/eff';
import * as JSX7 from '@eslint-react/jsx';
import { createRuleForPlugin, getSettingsFromContext, unsafeDecodeSettings } from '@eslint-react/shared';
import * as AST11 from '@eslint-react/ast';
import { isForwardRefCall, isThisSetState, isClassComponent, isCreateElementCall, isCloneElementCall, isChildrenCount, isChildrenForEach, isChildrenMap, isChildrenOnly, isChildrenToArray, useComponentCollectorLegacy, isCreateRefCall, isComponentName, useComponentCollector, ERFunctionComponentFlag, isChildrenToArrayCall, ERComponentHint, isDirectValueOfRenderPropertyLoose, isDeclaredInRenderPropLoose, isInsideCreateElementProps, isInsideRenderMethod, ERClassComponentFlag, isReactHookCall, isInitializedFromReact } from '@eslint-react/core';
...
function getName(node) {
  if (AST11.isTypeExpression(node)) {
...
var no_context_provider_default = createRule({
  ...
  create(context) {
    ...
    return {
      JSXElement(node) {
        const elementName = JSX7.getElementName(node.openingElement);
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        if (O.isSome(AST11.findParentNode(node, isClassComponent))) return;
...
var no_direct_mutation_state_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      ...
      return F.pipe(
        O.Do,
        O.bind("parentClass", () => AST11.findParentNodeGuard(
          node,
          AST11.isOneOf([
        ...
        O.bind("parentConstructor", () => AST11.findParentNodeGuard(node, isConstructorFunction)),
        O.filter(
          ({ parentClass, parentConstructor }) => isClassComponent(parentClass) && context.sourceCode.getScope(node).block !== parentConstructor
        ),
        O.map(() => ({
...
var no_duplicate_key_default = createRule({
  ...
  create(context) {
    ...
    function isKeyValueEqual(a, b) {
      ...
      return AST11.isNodeEqual(aValue, bValue);
    }
    return {
        ...
        switch (jsxElement.parent.type) {
          ...
          default: {
            const mbEntry = F.pipe(
              O.Do,
              O.bind("call", () => AST11.findParentNodeGuard(jsxElement, AST11.isMapCallLoose)),
              O.bind("iter", ({ call: call2 }) => AST11.findParentNodeStop(jsxElement, call2, AST11.isFunction)),
              O.bind("arg0", ({ call: call2 }) => O.fromNullable(call2.arguments[0]))
            );
            if (O.isNone(mbEntry)) return;
            const { arg0, call, iter } = mbEntry.value;
            if (AST11.unwrapTypeExpression(arg0) !== iter) return;
...
var no_implicit_key_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      ...
      const keyPropOnElement = node.attributes.some((n) => AST11.is(AST_NODE_TYPES.JSXAttribute)(n) && n.name.name === "key");
  ...
  create(context) {
    ...
    function checkBlockStatement(node) {
      return AST11.getNestedReturnStatements(node).reduce((acc, statement) => {
        ...
        return O.isNone(mbDescriptor) ? acc : [...acc, mbDescriptor.value];
      }, []);
    }
    return {
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        for (const { name: componentName, node: component } of functionComponents) {
          ...
          const isInsideJSXPropValue = component.parent.type === AST_NODE_TYPES.JSXAttribute || O.isSome(JSX7.findParentProp(node, (n) => n.value?.type === AST_NODE_TYPES.JSXExpressionContainer));
        ...
        for (const { name: name2, node: component } of classComponents) {
          if (O.isNone(AST11.findParentNode(component, or(isClassComponent11, isFunctionComponent)))) {
            continue;
          }
          context.report({
            messageId: "nestedComponent",
...
var no_set_state_in_component_did_mount_default = createRule({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      ...
      return F.pipe(
        O.Do,
        O.bind("clazz", () => AST11.findParentNodeGuard(node, isClassComponent)),
        O.bind("method", ({ clazz }) => AST11.findParentNodeStop(node, clazz, isComponentDidMount)),
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      ...
      return F.pipe(
        O.Do,
        O.bind("clazz", () => AST11.findParentNodeGuard(node, isClassComponent)),
        O.bind("method", ({ clazz }) => AST11.findParentNodeStop(node, clazz, isComponentDidUpdate)),
        O.bind("methodScope", ({ method }) => O.some(context.sourceCode.getScope(method))),
        O.bind("upperScope", () => O.fromNullable(context.sourceCode.getScope(node).upper)),
        O.filter(
          ({
  ...
  create(context) {
    ...
    function getReportDescriptor(node) {
      ...
      return F.pipe(
        O.Do,
        O.bind("clazz", () => AST11.findParentNodeGuard(node, isClassComponent)),
        O.bind("method", ({ clazz }) => AST11.findParentNodeStop(node, clazz, isComponentWillUpdate3)),
        O.bind("methodScope", ({ method }) => O.some(context.sourceCode.getScope(method))),
        O.bind("upperScope", () => O.fromNullable(context.sourceCode.getScope(node).upper)),
        O.filter(
          ({
    ...
    return {
      ...
      JSXOpeningElement(node) {
        ...
        F.pipe(
          ...
          O.bind("attribute", () => O.fromNullable(
          ...
          O.bind("valueExpression", ({ value }) => value?.type === AST_NODE_TYPES.JSXExpressionContainer ? O.some(value.expression) : O.none()),
...
function checkAndReport(node, context, allowExpressions) {
  ...
  const isChildElement = AST11.isOneOf([AST_NODE_TYPES.JSXElement, AST_NODE_TYPES.JSXFragment])(node.parent);
  switch (true) {
    // <Foo content={<>ee eeee eeee ...</>} />
    case (allowExpressions && !isChildElement && node.children.length === 1 && JSX7.isLiteral(node.children.at(0))): {
      return;
    }
    // <Foo><>hello, world</></Foo>
...
var no_useless_fragment_default = createRule({
  ...
  create(context, [option]) {
    ...
    return {
      JSXElement(node) {
        if (JSX7.getElementName(node.openingElement).split(".").at(-1) !== "Fragment") return;
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        function isFunctionComponent(block) {
          ...
          return O.exists(
            AST11.getFunctionIdentifier(block),
            (id) => isComponentName(id.name) && components.some((component) => component.node === block)
          );
        }
...
var prefer_shorthand_fragment_default = createRule({
  ...
  create(context) {
    function getReportDescriptor(node) {
      if (JSX7.getElementName(node.openingElement).split(".").at(-1) !== "Fragment") return O.none();
  ...
  create(context) {
    ...
    return {
      JSXOpeningElement(node) {
        ...
        for (const name2 of O.toArray(getName5(node.name))) {

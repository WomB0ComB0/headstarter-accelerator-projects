...
</html>`,$r=h("/error",{method:"GET",metadata:{...X,openapi:{description:"Displays an error page",responses:{200:{description:"Success",content:{"text/html":{schema:{type:"string"}}}}}}}},async e=>{let t=new URL(e.request?.url||"").searchParams.get("error")||"Unknown";return new Response(Nr(t),{headers:{"Content-Type":"text/html"}})});var Br=h("/ok",{method:"GET",metadata:{...X,openapi:{description:"Check if the API is working",responses:{200:{description:"Success",content:{"application/json":{schema:{type:"object",properties:{ok:{type:"boolean"}}}}}}}}}},async e=>e.json({ok:!0}));import{z as Ma}from"zod";import{APIError as Qa}from"better-call";import{z as K}from"zod";import{APIError as fe}from"better-call";var Vr=h("/list-accounts",{method:"GET",use:[D],metadata:{openapi:{description:"List all accounts linked to the user",responses:{200:{description:"Success",content:{"application/json":{schema:{type:"array",items:{type:"object",properties:{id:{type:"string"},provider:{type:"string"}}}}}}}}}}},async e=>{let t=e.context.session,r=await e.context.internalAdapter.findAccounts(t.user.id);return e.json(r.map(o=>({id:o.id,provider:o.providerId,createdAt:o.createdAt,updatedAt:o.updatedAt,accountId:o.accountId,scopes:o.scope?.split(",")||[]})))}),Mr=h("/link-social",{method:"POST",requireHeaders:!0,query:K.object({currentURL:K.string().optional()}).optional(),body:K.object({callbackURL:K.string({description:"The URL to redirect to after the user has signed in"}).optional(),provider:K.enum(Ee,{description:"The OAuth2 provider to use"})}),use:[D],metadata:{openapi:{description:"Link a social account to the user",responses:{200:{description:"Success",content:{"application/json":{schema:{type:"object",properties:{url:{type:"string"},redirect:{type:"boolean"}},required:["url","redirect"]}}}}}}}},async e=>{let t=e.context.session;if((await e.context.internalAdapter.findAccounts(t.user.id)).find(c=>c.providerId===e.body.provider))throw new fe("BAD_REQUEST",{message:u.SOCIAL_ACCOUNT_ALREADY_LINKED});let n=e.context.socialProviders.find(c=>c.id===e.body.provider);if(!n)throw e.context.logger.error("Provider not found. Make sure to add the provider in your auth config",{provider:e.body.provider}),new fe("NOT_FOUND",{message:u.PROVIDER_NOT_FOUND});let i=await ue(e,{userId:t.user.id,email:t.user.email}),s=await n.createAuthorizationURL({state:i.state,codeVerifier:i.codeVerifier,redirectURI:`${e.context.baseURL}/callback/${n.id}`});return e.json({url:s.toString(),redirect:!0})}),qr=h("/unlink-account",{method:"POST",body:K.object({providerId:K.string()}),use:[ct]},async e=>{let t=await e.context.internalAdapter.findAccounts(e.context.session.user.id);if(t.length===1)throw new fe("BAD_REQUEST",{message:u.FAILED_TO_UNLINK_LAST_ACCOUNT});if(!t.find(o=>o.providerId===e.body.providerId))throw new fe("BAD_REQUEST",{message:u.ACCOUNT_NOT_FOUND});return await e.context.internalAdapter.deleteAccount(e.body.providerId,e.context.session.user.id),e.json({status:!0})});import{APIError as dt}from"better-call";import{createHMAC as Fr}from"@better-auth/utils/hmac";var Yc=e=>({id:"bearer",hooks:{before:[{matcher(t){return!!(t.request?.headers.get("authorization")||t.headers?.get("authorization"))},handler:async t=>{let r=t.request?.headers.get("authorization")?.replace("Bearer ","")||t.headers?.get("authorization")?.replace("Bearer ","");if(!r)return;let o="";if(r.includes("."))o=r.replace("=","");else{if(e?.requireSignature)return;o=(await zr("",r,t.context.secret)).replace("=","")}try{let n=decodeURIComponent(o);if(!await Fr("SHA-256","base64urlnopad").verify(t.context.secret,n.split(".")[0],n.split(".")[1]))return}catch{return}return t.request&&t.request.headers.append("cookie",`${t.context.authCookies.sessionToken.name}=${o}`),t.headers&&t.headers.append("cookie",`${t.context.authCookies.sessionToken.name}=${o}`),{context:t}}}],after:[{matcher(t){return!!t.responseHeader.get("set-cookie")},handler:G(async t=>{let r=t.responseHeader.get("set-cookie");if(!r)return;let o=Oe(r),n=t.context.authCookies.sessionToken.name,i=o.get(n);if(!i||!i.value||i["max-age"]===0)return;let s=i.value;return t.responseHeader.set("set-auth-token",s),{responseHeader:t.responseHeader}})}]}});export{Yc as bearer};

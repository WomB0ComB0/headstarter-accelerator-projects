...
</html>`,eo=g("/error",{method:"GET",metadata:{...re,openapi:{description:"Displays an error page",responses:{200:{description:"Success",content:{"text/html":{schema:{type:"string"}}}}}}}},async e=>{let t=new URL(e.request?.url||"").searchParams.get("error")||"Unknown";return new Response(Xr(t),{headers:{"Content-Type":"text/html"}})});var to=g("/ok",{method:"GET",metadata:{...re,openapi:{description:"Check if the API is working",responses:{200:{description:"Success",content:{"application/json":{schema:{type:"object",properties:{ok:{type:"boolean"}}}}}}}}}},async e=>e.json({ok:!0}));var ro=require("zod");var oo=require("better-call");var Q=require("zod");var ue=require("better-call");var no=g("/list-accounts",{method:"GET",use:[C],metadata:{openapi:{description:"List all accounts linked to the user",responses:{200:{description:"Success",content:{"application/json":{schema:{type:"array",items:{type:"object",properties:{id:{type:"string"},provider:{type:"string"}}}}}}}}}}},async e=>{let t=e.context.session,r=await e.context.internalAdapter.findAccounts(t.user.id);return e.json(r.map(o=>({id:o.id,provider:o.providerId,createdAt:o.createdAt,updatedAt:o.updatedAt,accountId:o.accountId,scopes:o.scope?.split(",")||[]})))}),io=g("/link-social",{method:"POST",requireHeaders:!0,query:Q.z.object({currentURL:Q.z.string().optional()}).optional(),body:Q.z.object({callbackURL:Q.z.string({description:"The URL to redirect to after the user has signed in"}).optional(),provider:Q.z.enum(Pe,{description:"The OAuth2 provider to use"})}),use:[C],metadata:{openapi:{description:"Link a social account to the user",responses:{200:{description:"Success",content:{"application/json":{schema:{type:"object",properties:{url:{type:"string"},redirect:{type:"boolean"}},required:["url","redirect"]}}}}}}}},async e=>{let t=e.context.session;if((await e.context.internalAdapter.findAccounts(t.user.id)).find(c=>c.providerId===e.body.provider))throw new ue.APIError("BAD_REQUEST",{message:u.SOCIAL_ACCOUNT_ALREADY_LINKED});let n=e.context.socialProviders.find(c=>c.id===e.body.provider);if(!n)throw e.context.logger.error("Provider not found. Make sure to add the provider in your auth config",{provider:e.body.provider}),new ue.APIError("NOT_FOUND",{message:u.PROVIDER_NOT_FOUND});let i=await we(e,{userId:t.user.id,email:t.user.email}),s=await n.createAuthorizationURL({state:i.state,codeVerifier:i.codeVerifier,redirectURI:`${e.context.baseURL}/callback/${n.id}`});return e.json({url:s.toString(),redirect:!0})}),so=g("/unlink-account",{method:"POST",body:Q.z.object({providerId:Q.z.string()}),use:[Lt]},async e=>{let t=await e.context.internalAdapter.findAccounts(e.context.session.user.id);if(t.length===1)throw new ue.APIError("BAD_REQUEST",{message:u.FAILED_TO_UNLINK_LAST_ACCOUNT});if(!t.find(o=>o.providerId===e.body.providerId))throw new ue.APIError("BAD_REQUEST",{message:u.ACCOUNT_NOT_FOUND});return await e.context.internalAdapter.deleteAccount(e.body.providerId,e.context.session.user.id),e.json({status:!0})});var De=require("better-call");var jt={OTP_NOT_ENABLED:"OTP not enabled",OTP_HAS_EXPIRED:"OTP has expired",TOTP_NOT_ENABLED:"TOTP not enabled",TWO_FACTOR_NOT_ENABLED:"Two factor isn't enabled",BACKUP_CODES_NOT_ENABLED:"Backup codes aren't enabled",INVALID_BACKUP_CODE:"Invalid backup code"};var Ce=()=>{let e={INVALID_USERNAME_OR_PASSWORD:"invalid username or password",EMAIL_NOT_VERIFIED:"email not verified",UNEXPECTED_ERROR:"unexpected error",USERNAME_IS_ALREADY_TAKEN:"username is already taken. please try another."};return{id:"username",endpoints:{signInUsername:g("/sign-in/username",{method:"POST",body:pe.z.object({username:pe.z.string({description:"The username of the user"}),password:pe.z.string({description:"The password of the user"}),rememberMe:pe.z.boolean({description:"Remember the user session"}).optional()}),metadata:{openapi:{summary:"Sign in with username",description:"Sign in with username",responses:{200:{description:"Success",content:{"application/json":{schema:{type:"object",properties:{user:{$ref:"#/components/schemas/User"},session:{$ref:"#/components/schemas/Session"}}}}}}}}}},async t=>{let r=await t.context.adapter.findOne({model:"user",where:[{field:"username",value:t.body.username.toLowerCase()}]});if(!r)throw await t.context.password.hash(t.body.password),t.context.logger.error("User not found",{username:Ce}),new X.APIError("UNAUTHORIZED",{message:e.INVALID_USERNAME_OR_PASSWORD});if(!r.emailVerified&&t.context.options.emailAndPassword?.requireEmailVerification)throw await xe(t,r),new X.APIError("UNAUTHORIZED",{message:e.INVALID_USERNAME_OR_PASSWORD});let o=await t.context.adapter.findOne({model:"account",where:[{field:"userId",value:r.id},{field:"providerId",value:"credential"}]});if(!o)throw new X.APIError("UNAUTHORIZED",{message:e.INVALID_USERNAME_OR_PASSWORD});let n=o?.password;if(!n)throw t.context.logger.error("Password not found",{username:Ce}),new X.APIError("UNAUTHORIZED",{message:e.INVALID_USERNAME_OR_PASSWORD});if(!await t.context.password.verify({hash:n,password:t.body.password}))throw t.context.logger.error("Invalid password"),new X.APIError("UNAUTHORIZED",{message:e.INVALID_USERNAME_OR_PASSWORD});let s=await t.context.internalAdapter.createSession(r.id,t.request,t.body.rememberMe===!1);return s?(await I(t,{session:s,user:r},t.body.rememberMe===!1),t.json({token:s.token,user:{id:r.id,email:r.email,emailVerified:r.emailVerified,name:r.name,image:r.image,createdAt:r.createdAt,updatedAt:r.updatedAt}})):t.json(null,{status:500,body:{message:u.FAILED_TO_CREATE_SESSION,status:500}})})},schema:{user:{fields:{username:{type:"string",required:!1,unique:!0,returned:!0,transform:{input(t){return t?.toString().toLowerCase()}}}}}},hooks:{before:[{matcher(t){return t.path==="/sign-up/email"},async handler(t){let r=t.body.username;if(r&&await t.context.adapter.findOne({model:"user",where:[{field:"username",value:r.toLowerCase()}]}))throw new X.APIError("UNPROCESSABLE_ENTITY",{message:e.USERNAME_IS_ALREADY_TAKEN})}}]},$ERROR_CODES:jt}};0&&(module.exports={username});

...
function getSignalValueExpression(node, initialScope) {
  switch (node.type) {
    case utils.AST_NODE_TYPES.Identifier:
      return eff.F.pipe(
        VAR2__namespace.findVariable(node, initialScope),
        eff.O.flatMap(VAR2__namespace.getVariableNode(0)),
        eff.O.flatMap((n) => getSignalValueExpression(n, initialScope))
      );
    ...
    return {
      ...
      ["Program:exit"]() {
        for (const aEntry of aEntries) {
          if (eff.O.exists(aEntry.signal, (signal) => abortedSignals.some((as) => isSameObject(as, signal)))) continue;
    ...
    return {
      ...
      ["Program:exit"]() {
        for (const [node, id, _, phaseNode] of observers) {
          ...
          const hasDynamicallyAdded = oentries.some((e) => eff.O.exists(AST2__namespace.findParentNode(e.node, eff.or(isDynamic, isPhaseNode)), eff.not(isPhaseNode)));

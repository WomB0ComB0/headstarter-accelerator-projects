...
import { ERPhaseRelevance, isSetupFunction, isCleanupFunction, isComponentDidMountFunction, isComponentWillUnmountFunction, isInversePhase } from '@eslint-react/core';
import { O, F, or, not, isBoolean } from '@eslint-react/eff';
...
function getSignalValueExpression(node, initialScope) {
  switch (node.type) {
    case AST_NODE_TYPES.Identifier:
      return F.pipe(
        VAR2.findVariable(node, initialScope),
    ...
    return {
      ...
      ["Program:exit"]() {
        for (const [node, id, _, phaseNode] of observers) {
          ...
          const hasDynamicallyAdded = oentries.some((e) => O.exists(AST2.findParentNode(e.node, or(isDynamic, isPhaseNode)), not(isPhaseNode)));

...
Add the file in: "${config.routesDirectory}/${rootPathId}.${config.disableTypes ? "js" : "tsx"}"`;
  ...
  const handleRootNode = async (node) => {
    ...
    /* custom-patch-cn1-start *//*00001*/let routeCode; try { try { process.emit('ninja-fs-suspend'); } catch (e) { } routeCode = fs.readFileSync(node.fullPath, "utf-8"); } finally { try { process.emit('ninja-fs-resume'); } catch (e) { } }/* custom-patch-cn1-end */ // const routeCode = fs.readFileSync(node.fullPath, "utf-8");
    if (!routeCode) {

    ...
    try {
      const arg = await (async () => {
        ...
        if (method.toLowerCase() === "get") {
          ...
          return reactRouter.defaultTransformer.parse(search.payload);
        }
        const jsonPayloadAsString = await request.text();
        return reactRouter.defaultTransformer.parse(jsonPayloadAsString);
      })();
      const result = await action(arg);
      if (result instanceof Response) {
        return result;
      } else if (reactRouter.isPlainObject(result) && "result" in result && result.result instanceof Response) {
      ...
      return new Response(
        result !== void 0 ? reactRouter.defaultTransformer.stringify(result) : void 0,
        {
      ...
      if (error instanceof Response) {
        return error;
      } else if (reactRouter.isPlainObject(error) && "result" in error && error.result instanceof Response) {
        return error.result;
      }
      ...
      console.error(error);
      console.info();
      return new Response(reactRouter.defaultTransformer.stringify(error), {
        status: 500,
        headers: {
          "Content-Type": "application/json"
        }

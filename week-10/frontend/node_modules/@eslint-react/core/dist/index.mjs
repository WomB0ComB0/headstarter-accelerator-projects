...
import { getSettingsFromContext, unsafeDecodeSettings } from '@eslint-react/shared';
...
function isInitializedFromReact(name, initialScope, importSource = "react") {
  return name.toLowerCase() === "react" || VAR.isInitializedFromSource(
    name,
    importSource,
    initialScope
  );
  ...
  return (node, context) => {
    ...
    if (!settings.strictImportCheck) {
      if (node.type === AST_NODE_TYPES.MemberExpression) {
        return node.object.type === AST_NODE_TYPES.Identifier && node.property.type === AST_NODE_TYPES.Identifier && node.property.name === name;
      }
      return node.name === name;
    }
    const initialScope = context.sourceCode.getScope(node);
    if (node.type === AST_NODE_TYPES.MemberExpression) {
      return node.object.type === AST_NODE_TYPES.Identifier && node.property.type === AST_NODE_TYPES.Identifier && node.property.name === name && isInitializedFromReact(node.object.name, initialScope, settings.importSource);
    }
    if (node.name === name) {
...
function isFromReactMember(memberName, name) {
  return (node, context) => {
    ...
    if (!settings.strictImportCheck) {
      ...
      if (node.object.type === AST_NODE_TYPES.Identifier && node.object.name === memberName) return true;
      if (node.object.type === AST_NODE_TYPES.MemberExpression && node.object.object.type === AST_NODE_TYPES.Identifier && node.object.property.type === AST_NODE_TYPES.Identifier) {
    ...
    if (node.object.type === AST_NODE_TYPES.MemberExpression && node.object.object.type === AST_NODE_TYPES.Identifier && isInitializedFromReact(node.object.object.name, initialScope, settings.importSource) && node.object.property.type === AST_NODE_TYPES.Identifier) {
      return node.object.property.name === memberName;
    }
    return false;
  };
...
function isInsideCreateElementProps(node, context) {
  return F.pipe(
    O.Do,
    O.bind("call", () => AST9.findParentNodeGuard(node, isCreateElementCall(context))),
    O.bind("prop", () => AST9.findParentNodeGuard(node, AST9.is(AST_NODE_TYPES.ObjectExpression))),
...
function isReactHook(node) {
  return F.pipe(
    ...
    O.flatMapNullable((id2) => id2.name),
...
function isInsideReactHook(node) {
  return O.exists(AST9.findParentNodeGuard(node, AST9.isFunction), isReactHook);
}
function useHookCollector() {
  const hooks = /* @__PURE__ */ new Map();
  const fStack = [];
  const onFunctionEnter = (node) => {
    const id2 = AST9.getFunctionIdentifier(node);
    const name = O.flatMapNullable(id2, (id3) => id3.name);
    if (O.isSome(id2) && O.isSome(name) && isReactHookName(name.value)) {
      const key = getId();
  ...
  const listeners = {
    ...
    "CallExpression[type]"(node) {
      ...
      F.pipe(
        O.Do,
        O.bind("id", () => hookId),
        O.bind("hook", ({ id: id2 }) => O.fromNullable(hooks.get(id2))),
        O.map(({ id: id2, hook }) => {
          hooks.set(id2, {
            ...hook,
            hookCalls: [
...
function useComponentCollectorLegacy() {
  ...
  const collect = (node) => {
    ...
    const id2 = AST9.getClassIdentifier(node);
    const key = getId();
    ...
    components.set(
      key,
      {
        ...
        id: id2,
        kind: "class",
        name: O.flatMapNullable(id2, (n) => n.name),
        node,
...
function isInsideRenderMethod(node) {
  return O.isSome(AST9.findParentNode(node, (node2) => {
...
function useComponentCollector(context, hint = DEFAULT_COMPONENT_HINT) {
  ...
  const onFunctionExit = () => {
    ...
    const shouldDrop = AST9.getNestedReturnStatements(node.body).slice().reverse().some((r) => {
      return context.sourceCode.getScope(r).block === node && r.argument !== null && !JSX4.isJSXValue(r.argument, jsxCtx, hint);
    });
  ...
  const listeners = {
    ...
    "ArrowFunctionExpression[type][body.type!='BlockStatement']"() {
      ...
      const isComponent = hasNoneOrValidComponentName(entry.node, context) && JSX4.isJSXValue(body, jsxCtx, hint) && hasValidHierarchy(entry.node, context, hint);
      if (!isComponent) return;
      const initPath = AST9.getFunctionInitPath(entry.node);
      const id2 = getFunctionComponentIdentifier(entry.node, context);
      const name = O.flatMapNullable(id2, getComponentNameFromIdentifier);
      const key = getId();
      components.set(key, {
        _: key,
        id: id2,
    ...
    "AssignmentExpression[type][operator='='][left.type='MemberExpression'][left.property.name='displayName']"(node) {
      ...
      const mbComponentName = match(left.object).with({ type: AST_NODE_TYPES.Identifier }, (n) => O.some(n.name)).otherwise(O.none);
      if (O.isNone(mbComponentName)) return;
      const componentName = mbComponentName.value;
      const component = Array.from(components.values()).findLast(({ name }) => O.exists(name, (n) => n === componentName));
      if (!component) return;
    ...
    "ReturnStatement[type]"(node) {
      ...
      if (O.isNone(mbEntry)) return;
      const entry = mbEntry.value;
      const isComponent = hasNoneOrValidComponentName(entry.node, context) && JSX4.isJSXValue(node.argument, jsxCtx, hint) && hasValidHierarchy(entry.node, context, hint);
      if (!isComponent) return;
      functionEntries.pop();
...
function isCleanupFunction(node) {
  const nearestRet = O.getOrNull(AST9.findParentNodeGuard(node, AST9.is(AST_NODE_TYPES.ReturnStatement)));
  if (!nearestRet) return false;
  const nearestFunction = O.getOrNull(AST9.findParentNodeGuard(node, AST9.isFunction));
  const nearestFunctionOfRet = O.getOrNull(AST9.findParentNodeGuard(nearestRet, AST9.isFunction));
...
function isDeclaredInRenderPropLoose(node) {
  ...
  return F.pipe(
    AST9.findParentNodeGuard(node, AST9.is(AST_NODE_TYPES.JSXExpressionContainer)),
...
function isRenderFunctionLoose(node, context) {
  ...
  if (O.exists(AST9.getFunctionIdentifier(node), (id2) => id2.name.startsWith("render"))) {

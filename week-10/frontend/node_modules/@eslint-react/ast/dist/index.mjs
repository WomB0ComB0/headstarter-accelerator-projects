import { F, O, or, isString } from '@eslint-react/eff';
...
var findParentNode = F.dual(2, (node, test) => {
...
var findParentNodeGuard = F.dual(2, (node, predicate) => {
  let parent = node.parent;
  while (parent) {
    if (predicate(parent)) {
      return O.some(parent);
...
function findParentNodeStop(node, stopNode, predicate) {
  let parent = node.parent;
  while (parent && parent !== stopNode && parent.type !== AST_NODE_TYPES.Program) {
    if (predicate(parent)) {
      return O.some(parent);
    }
    parent = parent.parent;
...
function getNestedExpressionsOfType(type) {
  ...
  return function(node) {
    ...
    if (node.type === AST_NODE_TYPES$1.TSSatisfiesExpression) {
      const chunk = boundGetNestedExpressionsOfType(node.expression);
      expressions.push(...chunk);
    }
    return expressions;
...
function getNestedReturnStatements(node) {
  ...
  simpleTraverse(node, {
    enter(node2) {
      ...
      const parentFunction = O.getOrNull(findParentNodeGuard(node2, isFunction));
